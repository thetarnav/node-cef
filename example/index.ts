...
<html>
<head>
    <title>Hello CEF BUN Example</title>
    <style>
        body {
...
<body>
    ...
    <script>
        ...
        function sendMessage() {
            sendToNative(JSON.stringify({ type: 'greeting', data: ++counter }));
        }
        function sendCustom() {
            sendToNative(JSON.stringify({ type: 'custom', payload: { timestamp: Date.now(), random: Math.random() } }));
        }
        function sendInput() {
            const text = document.getElementById('input').value;
            sendToNative(JSON.stringify({ type: 'user-input', text }));
        }
